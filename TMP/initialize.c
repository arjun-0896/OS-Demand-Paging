...
/* To set the interrupt service routine as "pfintr" on pagefault and inames[14] = "pagefault" */
set_evec(14, pfintr);
    
/* Initialization Fifo queue head and allocated frames(FR_PAGE) */
F_head.fid = -1;
F_head.next_frame = NULL;
n_allocated_pages = 0;
/* ++++++++++++++++++++++++++++++++++++++++++++ */

    
/* Demand paging function initialization */
init_bsm();
init_frm();
/* initialize first 4 page tables from 1024 as global page tables */
initialize_globalPageTables();
/* initialize null process by creating directory and page tables and updating cr3 with the pdbr */
create_page_directory(NULLPROC);
create_page_table(NULLPROC);
update_cr3(NULLPROC);
enable_paging();
/*-------------------------------------  */
...
*/
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	pptr->pprio = 0;
	currpid = NULLPROC;
/* initializing process datastructures needed for demand paging */
    pptr->store = -1;
    pptr->vhpno = -1;
    pptr->vhpnpages = -1;
    pptr->vmemlist = NULL;
    for (i = 0; i < NBACKINGSTORE; i++) {
        pptr->backing_store_map[i].bs_isprivheap = 0;
        pptr->backing_store_map[i].bs_npages = 0;
        pptr->backing_store_map[i].bs_pid = -1;
        pptr->backing_store_map[i].bs_refcnt = 0;
        pptr->backing_store_map[i].bs_sem = -1;
        pptr->backing_store_map[i].bs_status = 0;
        pptr->backing_store_map[i].bs_vpno = -1;
    }
/*............................................................ */
